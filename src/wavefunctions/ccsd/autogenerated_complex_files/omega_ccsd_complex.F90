...
!
      req1_a = 0
      req1_b = 0
      call wf%eri_complex%get_eri_t1_mem('vvvv', req1_a, req1_b, 1, wf%n_v, 1, wf%n_v)
!
...
!
        call output%error_msg('Expected same-sized batches in omega_ccsd_a2_complex &
...
!
            call wf%eri_complex%get_eri_t1('vvvv', g_acbd,              &
...
!
      ...
      call wf%eri_complex%get_eri_t1('vovo', g_aibj)
...
!
      call wf%eri_complex%get_eri_t1('oooo', g_kilj)
!
...
!
      call wf%eri_complex%get_eri_t1('ovov', g_kcld)
!
...
!
      call wf%eri_complex%get_eri_t1('ovov', g_kdlc)
!
...
!
      req0 = wf%n_o**2*wf%eri_complex%n_J
!
      req1 = wf%n_v*wf%eri_complex%n_J + (wf%n_o)*(wf%n_v**2)
!
      batch_a = batching_index(wf%n_v)
!
...
!
         call wf%eri_complex%get_eri_t1('oovv', g_kiac, first_r=batch_a%first, last_r=batch_a%last)
!
...
!
      ...
      complex(dp), dimension(:,:,:), allocatable   :: L_J_kc, L_J_ai ! Cholesky vectors, term 1
      complex(dp), dimension(:,:,:), allocatable   :: X_bj_J         ! Intermediate, term 1
!
...
!
      ...
      call wf%eri_complex%get_eri_t1('ovov', g_ldkc)
!
...
!
      call mem%alloc(L_J_kc, wf%eri_complex%n_J, wf%n_o, wf%n_v)
      call wf%eri_complex%get_cholesky_t1(L_J_kc, 1, wf%n_o, wf%n_o + 1, wf%n_mo)
!
      call mem%alloc(X_bj_J, wf%n_v, wf%n_o, wf%eri_complex%n_J)
!
      call zgemm('N', 'T',           &
                  (wf%n_o)*(wf%n_v), &
                  wf%eri_complex%n_J,        &
                  ...
                  (wf%n_o)*(wf%n_v))
!
      call mem%dealloc(L_J_kc, wf%eri_complex%n_J, wf%n_o, wf%n_v)
!
      call mem%alloc(L_J_ai, wf%eri_complex%n_J, wf%n_v, wf%n_o)
      call wf%eri_complex%get_cholesky_t1(L_J_ai, wf%n_o + 1, wf%n_mo, 1, wf%n_o)
!
...
!
      call zgemm('T', 'T',           &
                  ...
                  (wf%n_o)*(wf%n_v))
!
      call mem%dealloc(X_bj_J, wf%n_v, wf%n_o, wf%eri_complex%n_J)
      call mem%dealloc(L_J_ai, wf%eri_complex%n_J, wf%n_v, wf%n_o)
