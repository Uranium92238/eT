...
!
      ...
      call wf%jacobian_transpose_ccs_b1_complex(sigma, b)
!
      call timer%turn_off()
!
...
!
      req0 = 0
!
      req1_a = max((wf%eri_complex%n_J)*(wf%n_v),(wf%eri_complex%n_J)*(wf%n_o))
      req1_j = max((wf%eri_complex%n_J)*(wf%n_v),(wf%eri_complex%n_J)*(wf%n_o))
!
...
!
            call wf%eri_complex%get_eri_t1('voov', L_bjia, 1, wf%n_v, batch_j%first, batch_j%last, &
...
!
            call wf%eri_complex%get_eri_t1('vvoo', g_baij, 1, wf%n_v, batch_a%first, batch_a%last, &
