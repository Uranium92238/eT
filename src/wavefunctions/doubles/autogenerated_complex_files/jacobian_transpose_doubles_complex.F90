...
!
      ...
      complex(dp), dimension(:,:,:,:), allocatable :: L_aick
      complex(dp), dimension(:,:,:,:), allocatable :: u_cjbk
!
      complex(dp), dimension(:,:), allocatable :: X_ck, Y_ik, Y_ca
!
      type(timings) :: timer
!
...
!
      ...
      call zero_array_complex(L_aick, wf%n_t1**2)
!
      call add_2143_to_1234(two_complex, g_iakc, L_aick, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
      call add_2341_to_1234(-one_complex, g_iakc, L_aick, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
...
!     sigma_ai =+ - c_ci * Y_ca
!
      call zgemm('T', 'N',    & ! transpose of Y_ik
                  wf%n_v,     &
                  wf%n_o,     &
                  wf%n_v,     &
                  -one_complex,       &
...
!
                  sigma_aibj(a, i, b, j) = sigma_aibj(a, i, b, j) &
                                          +  two_complex*wf%fock_ia_complex(j, b)*c_ai(a, i)&
                                          -  wf%fock_ia_complex(i, b)*c_ai(a, j)
