...
!
!
module gl4_cc_propagation_class
!
!!
!! gl4 CC propagation solver class
!! Written by Alice Balbi and Andreas Skeidsvoll, Jan 2019
!!
!! Makes an Gauss-Legendre 4 solver by letting the propagation solver do Gauss-Legendre 4 steps.
!!
...
!
   implicit none
!
   type, extends(cc_propagation) :: gl4_cc_propagation
!
...
!
      complex(dp), dimension(:), allocatable :: z1_guess, z2_guess
!
   contains
!
...
!
!
   interface gl4_cc_propagation
!
      procedure :: new_gl4_cc_propagation
!
   end interface gl4_cc_propagation
!
!
contains
!
!
   function new_gl4_cc_propagation(wf) result(solver)
!!
!!    New gl4 CC propagation
!!    Written by Andreas Skeidsvoll, Sep 2019
!!
      implicit none
!
      class(ccs) :: wf
      type(gl4_cc_propagation) :: solver
!
      solver%tag = 'gl4'
      solver%name_ = 'Gauss-Legendre 4 coupled cluster propagation solver'
!
...
!
      complex(dp), dimension(:), allocatable :: ddt_u1, ddt_u2, z1, z2, z1_next, z2_next
!
      integer :: i
!
      logical :: converged
...
!
         call solver%update_field_and_wavefunction(wf, field, ti+c1_gl4*dt, ui+z1)
         call wf%construct_complex_time_derivative(ddt_u1)
!
!        Construct derivative of the second stage
!
         call solver%update_field_and_wavefunction(wf, field, ti+c2_gl4*dt, ui+z2)
...
!
!        z1_next = dt*a11_gl4*ddt_u1 + dt*a12_gl4*ddt_u2
!
         call zcopy(n, ddt_u1, 1, z1_next, 1)
         call zdscal(n, dt*a11_gl4, z1_next, 1)
         call zaxpy(n, cmplx(dt*a12_gl4, zero, dp), ddt_u2, 1, z1_next, 1)
!
!        Construct second z vector
!
!        z2_next = dt*a21_gl4*ddt_u1 + dt*a22_gl4*ddt_u2
!
         call zcopy(n, ddt_u1, 1, z2_next, 1)
         call zdscal(n, dt*a21_gl4, z2_next, 1)
         call zaxpy(n, cmplx(dt*a22_gl4, zero, dp), ddt_u2, 1, z2_next, 1)
!
...
!
         if ((norm2(real(z1_next-z1))  .le. solver%implicit_threshold) &
             ...
             .and. (norm2(aimag(z2_next-z2)) .le. solver%implicit_threshold)) then
!
            converged = .true.
!
         endif
...
!
!     Use the derivative of the two stages to estimate the solution uf at time t = ti + dt.
!
!     uf = ui + dt*b1_gl4*ddt_u1 + dt*b2_gl4*ddt_u2
!
      call zcopy(n, ui, 1, uf, 1)
      call zaxpy(n, cmplx(dt*b1_gl4, zero, dp), ddt_u1, 1, uf, 1)
      call zaxpy(n, cmplx(dt*b2_gl4, zero, dp), ddt_u2, 1, uf, 1)
!
!     Calculate guess of next z vectors
!
!     solver%z1_guess = ui - uf + dt*beta11_gl4*ddt_u1 + dt*beta12_gl4*ddt_u2
!
      call zcopy(n, ui, 1, solver%z1_guess, 1)
      call zaxpy(n, -one_complex, uf, 1, solver%z1_guess, 1)
      call zaxpy(n, cmplx(dt*beta11_gl4, zero, dp), ddt_u1, 1, solver%z1_guess, 1)
      call zaxpy(n, cmplx(dt*beta12_gl4, zero, dp), ddt_u2, 1, solver%z1_guess, 1)
!
!     solver%z2_guess = ui - uf + dt*beta21_gl4*ddt_u1 + dt*beta22_gl4*ddt_u2
!
      call zcopy(n, ui, 1, solver%z2_guess, 1)
      call zaxpy(n, -one_complex, uf, 1, solver%z2_guess, 1)
      call zaxpy(n, cmplx(dt*beta21_gl4, zero, dp), ddt_u1, 1, solver%z2_guess, 1)
      call zaxpy(n, cmplx(dt*beta22_gl4, zero, dp), ddt_u2, 1, solver%z2_guess, 1)
!
      call mem%dealloc(ddt_u1, n)
      call mem%dealloc(ddt_u2, n)
!
   end subroutine gl4_step
!
!
end module gl4_cc_propagation_class