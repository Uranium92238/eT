...
!
         call dgemm('N', 'T',                  &
                     ...
                     c_a_i,                    &
                     wf%n_v,                   &
                     X_ij_kb,                  & !X_kb_ij = X_k_bij
                     batch_length*(wf%n_o)**2, &
                     zero,                     &
                     rho_a_ijb,                &
                     wf%n_v)
!
         call deallocator(X_ij_kb, wf%n_o**2, batch_length*(wf%n_o))
!
...
!
 ...
 !               
         real(dp), dimension(:,:), allocatable :: Y_b_d
...
!
!        Y_b_d = sum_clk L_d_lck * c_b_lck 
!
         call allocator(Y_b_d, wf%n_v, wf%n_v)
!
         call dgemm('N', 'T',              &
                     wf%n_v,               &
                     wf%n_v,               &
...
!
         ...
         call deallocator(Y_b_d, wf%n_v, wf%n_v)
!
...
! 
                  do l = 1, wf%n_o
! 
                    dl = index_two(d, l, wf%n_o)
! 
                     L_l_ckd(l, ckd) = L_ck_dl(ck, dl)
! 
                  enddo
               enddo
            enddo
...
!
         real(dp), dimension(:,:), allocatable :: t_dl_bj
         real(dp), dimension(:,:), allocatable :: t_lck_b
...
!
         integer(i15) :: aib = 0, aij = 0, ckd = 0, lck = 0
!
...
!        
         ...
         call wf%read_double_amplitudes
!
         call allocator(t_lck_b, (wf%n_v)*((wf%n_o)**2), wf%n_v)
         t_lck_b = zero
!
         do k = 1, wf%n_o
            do l = 1, wf%n_o
...
!
         call dgemm('N', 'N',              &
                     ...
                     wf%n_v)
!
         call deallocator(t_lck_b, (wf%n_v)*((wf%n_o)**2), wf%n_v)
!
