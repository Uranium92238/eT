...
!
!$omp parallel do schedule(static) private(a,i,j,b)
      do a = 1, wf%n_v
         do i = 1, wf%n_o
            do j = 1, wf%n_o
               do b = 1, wf%n_v
!
                  g_abji(a,i,j,b) = g_abji(a,b,j,i)
!
               enddo
            enddo
         enddo
...
!
      call wf%get_vovo(g_aick,      &
                        ...
                        1, wf%n_o)
!
      call mem%alloc(u_aikc, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
      do a = 1, wf%n_v
         do i = 1, wf%n_o
            do c = 1, wf%n_v
...
!
      ...
      call mem%dealloc(u_aikc, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
!
      ...
      do j = 1, wf%n_o
         do c = 1, wf%n_v
            do k = 1, wf%n_o
               do b = 1, wf%n_v
!
                  L_jckb(j,c,k,b) = two * g_kcjb(k,c,j,b) - g_kbjc(k,b,j,c)
!
               enddo
            enddo
         enddo
...
!
      ...
      enddo
!
!     Intermediat X_ji = L_kcjb t^cb_ki = L_jckb g_ckbi
!
...
!
      call mem%alloc(X_ab, (wf%n_v), (wf%n_o))
!
      call dgemm('N', 'N',                    &
                  (wf%n_v),                   &
                  (wf%n_v),                   &
...
!     Integrals
!
      real(dp), dimension(:,:,:,:), allocatable :: g_akbc
      real(dp), dimension(:,:,:,:), allocatable :: g_bjac
...
!     Intermediates
!
      real(dp), dimension(:,:,:,:), allocatable :: I_akbj
      real(dp), dimension(:,:,:,:), allocatable :: I_bjak
!
!     Indices
!
...
!
!     Construct I_akbj = sum_c g_akbc * c_cj
!
...
!
      call wf%get_vovv(g_akbc,    &
                        ...
                        1, wf%n_v)
!
      call mem%alloc(I_akbj, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
      call dgemm('N', 'N',              &
                  (wf%n_o)*(wf%n_v)**2, &
                  (wf%n_o),             &
...
!
!     Reordering of I_akbj as ajbk and scaling by (omega - ε_akbj)^-1 * (1 + delta_ak,bj)^-1
!
      do a = 1, wf%n_v
         do k = 1, wf%n_o
!
            I_akbj(a,k,a,k) = half*I_akbj(a,k,a,k)
!
               do b = 1, wf%n_v
                  do j = 1, wf%n_o
!
                     I_akbj(a,j,b,k) = - I_akbj(a,k,b,j) &
...
!
!     rho_ai = rho_ai - sum_jbk I_ajbk * L_jbki
!
      call dgemm('N', 'N',              &
                  (wf%n_v),             &
                  (wf%n_o),             &
...
!
      call wf%get_vovv(g_bjac,      &
                        ...
                        1, wf%n_v)
!
      call mem%alloc(I_bjak, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
      call dgemm('N', 'N',              &
                  (wf%n_v)**2*(wf%n_o), &
                  (wf%n_o),             &
...
!
!     Reordering of I_bjak as aj,bk and scaling by (omega - ε_akbj)^-1 * (1 + delta_ak,bj)^-1
!
      do a = 1, wf%n_v
         do k = 1, wf%n_o
!
            I_bjak(a,k,a,k) = half*I_bjak(a,k,a,k)
!
               do b = 1, wf%n_v
                  do j = 1, wf%n_o
!
                     I_bjak(a,j,b,k) = - I_bjak(b,j,a,k)&
                                       /(- eps_v(a) - eps_v(b) + eps_o(j) + eps_o(k) + omega)
!
                  enddo
               enddo
...
!
      call dgemm('N', 'N',              &
                  ...
                  (wf%n_v))
!
      call mem%dealloc(I_bjak, wf%n_v, wf%n_o, wf%n_v, wf%n_o)
!
      call mem%dealloc(L_jbki, wf%n_o, wf%n_v, wf%n_o, wf%n_o)
!
