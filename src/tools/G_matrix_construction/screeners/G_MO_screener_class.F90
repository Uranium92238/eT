...
!
   implicit none
!
   type, extends(abstract_G_screener) :: G_MO_screener
!
...
!
      procedure :: s1s2s3_cycle &
                => s1s2s3_cycle_G_MO_screener
!
      procedure :: s1s2s3s4_cycle &
                => s1s2s3s4_cycle_G_MO_screener
!
      procedure :: cleanup &
                => cleanup_G_MO_screener
!
!
      procedure :: get_D_max &
                => get_D_max_G_MO_screener
!
!     Overwritten routines
!
      procedure :: s1s2s3_exit &
                => s1s2s3_exit_G_MO_screener
!
      procedure :: prescreening &
                => prescreening_G_MO_screener
!
      procedure :: get_sig_s1s2 &
                => get_sig_s1s2_G_MO_screener
!
      procedure :: get_sig_s3 &
                => get_sig_s3_G_MO_screener
!
      procedure :: get_n_sig_s3 &
                => get_n_sig_s3_G_MO_screener
!
!     Private routines
!
...
!
   end type G_MO_screener
!
   interface  G_MO_screener
      procedure :: new_G_MO_screener
   end interface  G_MO_screener
!
contains
!
!
   function new_G_MO_screener(J_threshold, K_threshold, n_ao, n_mo, C) result(this)
!!
!!    New
...
!
   end function new_G_MO_screener
!
!
   pure function s1s2s3_cycle_G_MO_screener(this, ao, s1, s2, s3, s1s2) result(cycle_)
!!
!!    s1s2s3 cycle
...
!!
      implicit none
!
      class(G_MO_screener),intent(in) :: this
...
!
      if (density_K * cs_eri_max_s1s2s3 < this%K_threshold .and. &
          density_J * cs_eri_max_s1s2s3 < this%J_threshold) cycle_ = .true.
!
   end function s1s2s3_cycle_G_MO_screener
!
!
   pure function s1s2s3s4_cycle_G_MO_screener(this, ao, s1, s2, s3, s4, s1s2, s3s4) result(cycle_)
!!
!!    s1s2s3s4 cycle
!!    Written by Eirik F. Kjønstad, Linda Goletto, and Sarai D. Folkestad 2020-2021
!!
...
!
      density_K = max(this%shp_max_D(s3,s2) * this%sh_max_C(s1) * this%sh_max_C(s4),  &
                      this%shp_max_D(s3,s1) * this%sh_max_C(s2) * this%sh_max_C(s4),  &
...
!
!
   end function s1s2s3s4_cycle_G_MO_screener
!
!
   pure function get_D_max_G_MO_screener(this, s1, s2, s3, s4) result(D_max)
!
      implicit none
!
      class(G_MO_screener), intent(in)  :: this
...
!
   end function get_D_max_G_MO_screener
!
!
   subroutine cleanup_G_MO_screener(this)
!!
!!    Cleanup
!!    Written by Sarai D. Folkestad, 2021
!!
      implicit none
      class(G_MO_screener), intent(inout)  :: this
!
...
!
      this%C => null()
!
   end subroutine cleanup_G_MO_screener
!
!
   subroutine prescreening_G_MO_screener(this, ao, D)
!!
!!    Prescreening
!!    Written by Sarai D. Folkestad 2021
!!
...
!
   end subroutine prescreening_G_MO_screener
!
!
   pure function get_sig_s3_G_MO_screener(this, s3_tilde, s1) result(s3)
!!
!!    Get significant s3
!!    Written by Sarai D. Folkestad 2021
!!
...
!
   end function get_sig_s3_G_MO_screener
!
!
   pure function get_sig_s1s2_G_MO_screener(this, s1s2_tilde) result(s1s2)
!!
...
!
   end function get_sig_s1s2_G_MO_screener
!
!
   pure function get_n_sig_s3_G_MO_screener(this, s1) result(n_sig_s3)
!!
...
!
   end function get_n_sig_s3_G_MO_screener
!
!
...
!
      call mem%alloc(this%sig_s3, this%n_sh)
!
!$omp parallel do private(s3_tilde)
      do s3_tilde = 1, this%n_sig_s3
!
...
!
      ...
      real(dp) :: cs_eri_max_s1s2, density_J, density_K
!
...
!
      ...
      call mem%alloc(this%sig_s1s2, ao%n_sig_eri_shp, set_zero=.true.)
!
      this%n_sig_s1s2 = 0
!
...
!
         if (density_K * cs_eri_max_s1s2 < this%K_threshold .and. &
             density_J * cs_eri_max_s1s2 < this%J_threshold) cycle
!
...
!
!
   pure function s1s2s3_exit_G_MO_screener(this, ao, s1, s2, s3, s1s2) result(exit_)
!!
!!    s1s2s3 exit
!!    Written by Eirik F. Kjønstad, Linda Goletto, and Sarai D. Folkestad 2020-2021
!!
...
!
      logical :: exit_
!
      real(dp) :: D_max_shp_s1s2, cs_eri_max_s1s2, cs_eri_max_s1s2s3, density_J, density_K
!
...
!
      density_K = max(this%sh_max_D(s2)*this%sh_max_C(s1)*this%max_C,  &
                        this%sh_max_D(s1)*this%sh_max_C(s2)*this%max_C)
!
      s1s2_condition = (density_J * cs_eri_max_s1s2 < this%J_threshold .and. &
                        density_K * cs_eri_max_s1s2 < this%K_threshold)
!
...
!
   end function s1s2s3_exit_G_MO_screener
!
!
...
!
      ...
      if (allocated(this%sig_s3))call mem%dealloc(this%sig_s3, this%n_sh)
!
...
!
!
end module G_MO_screener_class
