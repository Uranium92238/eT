...
void construct_ao_q_wx(double *q_xx, double *q_xy, double *q_xz, double *q_yy, double *q_yz, double *q_zz, int *s1, int *s2){

  const auto& buf_vec = quadrupole.results(); // will point to computed shell sets

  auto n1 = basis[*s1 - 1].size();        // Number of basis functions in shell 1
  auto n2 = basis[*s2 - 1].size();        // number of basis functions in shell 2

  quadrupole.compute(basis[*s1 - 1], basis[*s2 - 1]);
