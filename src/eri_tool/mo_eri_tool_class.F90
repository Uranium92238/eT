...
!
!
module mo_eri_tool_class
!
!!
!!    MO eri tool class module
...
!
!
   type, extends(abstract_eri_tool) :: mo_eri_tool
!
...
!
      real(dp), dimension(:,:,:), allocatable :: L_J_oo_mo
      real(dp), dimension(:,:,:), allocatable :: L_J_vo_mo
      real(dp), dimension(:,:,:), allocatable :: L_J_ov_mo
      real(dp), dimension(:,:,:), allocatable :: L_J_vv_mo
!
!     Keep full electron repulsion matrix g_pqrs in memory? (+ allocatable arrays for keeping it)
!
      real(dp), dimension(:,:,:,:), allocatable :: g_pqrs_mo
!
   contains
!
!     Read, print settings
...
!
!
   interface mo_eri_tool
!
      procedure :: new_mo_eri_tool
      procedure :: new_mo_eri_tool_from_template
!
   end interface mo_eri_tool
!
!
contains
!
!
   function new_mo_eri_tool(n_o, n_v, n_J, need_g_abcd) result(eri)
!!
...
!
      if (eri%room_for_cholesky(dp)) then
!
...
!
   end function new_mo_eri_tool
!
!
   function new_mo_eri_tool_from_template(template) result(eri)
!!
...
!
      call eri%print_settings()
!
   end function new_mo_eri_tool_from_template
!
!
   subroutine print_settings_mo_eri_tool(eri)
!!
!!    Print settings
...
!
      call output%printf('m', &
                         ...
                         fs='(t6,a)')
!
   end subroutine print_settings_mo_eri_tool
!
!
   subroutine read_settings_mo_eri_tool(eri)
!!
!!    Read settings
!!    Written by Eirik F. Kjønstad, Dec 2019
!!
...
!
   end subroutine read_settings_mo_eri_tool
!
!
   subroutine initialize_mo_eri_tool(eri)
!!
!!    Initialize
...
!
   end subroutine initialize_mo_eri_tool
!
!
   subroutine copy_from_mo_mo_eri_tool(eri, template)
!!
!!    Copy from MO
!!    Written by Rolf H. Myhre, Aug 2020
!!
...
!
   end subroutine copy_from_mo_mo_eri_tool
!
!
   subroutine write_cholesky_mo_eri_tool(eri, L_J_pq, &
...
!!
      implicit none
!
      class(mo_eri_tool), intent(inout) :: eri
!
...
!
   end subroutine write_cholesky_mo_eri_tool
!
!
   subroutine read_cholesky_mo_eri_tool(eri, L_J_pq, &
                                     first_p, last_p, first_q, last_q, &
                                     file_)
!!
...
!
   end subroutine read_cholesky_mo_eri_tool
!
!
   subroutine copy_in_cholesky_mo_eri_tool(eri, L_J_pq, &
                                           first_p, last_p, first_q, last_q, &
...
!
      real(dp), dimension(eri%n_J, first_p:last_p, first_q:last_q), intent(in) :: L_J_pq
!
      real(dp), dimension(eri%n_J, eri%n_o, eri%n_o), intent(out) :: L_J_oo
      real(dp), dimension(eri%n_J, eri%n_v, eri%n_o), intent(out) :: L_J_vo
      real(dp), dimension(eri%n_J, eri%n_o, eri%n_v), intent(out) :: L_J_ov
      real(dp), dimension(eri%n_J, eri%n_v, eri%n_v), intent(out) :: L_J_vv
!
      integer :: i, j, a, b, K
!
!     oo block
...
!$omp end parallel do
!
   end subroutine copy_in_cholesky_mo_eri_tool
!
!
   subroutine copy_out_cholesky_mo_eri_tool(eri, L_J_pq, &
                                         first_p, last_p, first_q, last_q, &
                                         L_J_oo, L_J_vo, L_J_ov, L_J_vv)
!!
!!    copy out cholesky
...
!
      real(dp), dimension(eri%n_J, first_p:last_p, first_q:last_q), intent(out) :: L_J_pq
!
      real(dp), dimension(eri%n_J, eri%n_o, eri%n_o), intent(in) :: L_J_oo
      real(dp), dimension(eri%n_J, eri%n_v, eri%n_o), intent(in) :: L_J_vo
      real(dp), dimension(eri%n_J, eri%n_o, eri%n_v), intent(in) :: L_J_ov
      real(dp), dimension(eri%n_J, eri%n_v, eri%n_v), intent(in) :: L_J_vv
!
      integer :: i, j, a, b, K
!
!     oo block
...
!$omp end parallel do
!
   end subroutine copy_out_cholesky_mo_eri_tool
!
!
   subroutine set_cholesky_mo_mo_eri_tool(eri, L_J_pq, first_p, last_p, first_q, last_q)
!!
!!    Set Cholesky MO
!!    Written by Rolf H. Myhre, May 2020
!!
...
!
   end subroutine set_cholesky_mo_mo_eri_tool
!
!
   subroutine get_cholesky_mo_mo_eri_tool(eri, L_J_pq, first_p, last_p, first_q, last_q)
!!
!!    Get Cholesky MO
!!    Written by Rolf H. Myhre, May 2020
!!
...
!
   end subroutine get_cholesky_mo_mo_eri_tool
!
!
   subroutine update_cholesky_mo_mo_eri_tool(eri, T)
!!
!!    Update cholesky MO
!!    Written by Rolf H. Myhre, May 2020
!!
...
!
      class(mo_eri_tool), intent(inout) :: eri
!
      real(dp), dimension(eri%n_mo, eri%n_mo), intent(in) :: T
!
...
!
      call mem%batch_setup(batcher, 0, 2*eri%n_J*eri%n_mo, 'update_cholesky_mo')
...
!
      if (.not. all_in_mem) then
         temp_file = direct_stream_file('temp_file', eri%n_J, dp, 'new')
...
!
   end subroutine update_cholesky_mo_mo_eri_tool
!
!
   pure subroutine get_eri_mo_mem_mo_eri_tool(eri, string, req_pq, req_rs, &
...
!
         call eri%get_eri_mem(string, req_pq, req_rs, dim_p, dim_q, dim_r, dim_s, qp, sr)
!
      endif
!
   end subroutine get_eri_mo_mem_mo_eri_tool
!
!
   subroutine get_eri_mo_mo_eri_tool(eri, string, g_pqrs, &
                                     first_p, last_p, first_q, last_q, &
...
!
   end subroutine get_eri_mo_mo_eri_tool
!
!
   subroutine get_g_pqrs_mo_mo_eri_tool(eri, g_pqrs, first_p, last_p, first_q, last_q, &
                                                     first_r, last_r, first_s, last_s, &
                                                     alpha, beta, qp, sr, rspq)
!!
!!    Get g_pqrs MO
...
!
   end subroutine get_g_pqrs_mo_mo_eri_tool
!
!
   subroutine construct_g_pqrs_mo_mo_eri_tool(eri, g_pqrs, &
                                              first_p, last_p, first_q, last_q, &
                                              first_r, last_r, first_s, last_s, &
                                              alpha, beta, qp, sr, rspq)
!!
...
!
   end subroutine construct_g_pqrs_mo_mo_eri_tool
!
!
   subroutine copy_g_pqrs_mo_eri_tool(eri, g_to, g_from, &
                                      first_p, last_p, first_q, last_q, &
                                      first_r, last_r, first_s, last_s, &
                                      alpha, beta, qp, sr, rspq)
!!
...
!
      real(dp), dimension(:,:,:,:), pointer :: g_to_p => null()
!
      qp_ = .false.
      sr_ = .false.
      rspq_ = .false.
...
!
         call eri%construct_g_pqrs_mo(eri%g_pqrs_mo, 1, eri%n_mo, 1, eri%n_mo, &
                                                     1, eri%n_mo, 1, eri%n_mo, one, zero)
!
      endif
!
...
!
!
   subroutine construct_g_from_L_mo_eri_tool(eri, L_J_pq, L_J_rs, g_pqrs, alpha, beta, &
                                             dim_p, dim_q, dim_r, dim_s, rspq)
!!
!!    construct g from L
!!    written by Rolf H. Myhre, Jun 2020
...
!
      ...
      real(dp), dimension(eri%n_J*dim_r*dim_s), intent(in) :: L_J_rs
!
      real(dp), dimension(dim_p*dim_q*dim_r*dim_s), intent(inout) :: g_pqrs
!
      real(dp), intent(in) :: alpha, beta
!
...
!
      real(dp), intent(in) :: alpha, beta
!
      call dsfrk('N', 'U', 'T', dim_p*dim_q, eri%n_J, alpha, L_J_pq, eri%n_J, beta, g_pqpq)
!
...
!!
      implicit none
!
      class(mo_eri_tool), intent(in), target :: eri
...
!
   end subroutine set_pointer_mo_mo_eri_tool
!
!
   subroutine L_pointer_setup_mo_mo_eri_tool(eri, point, switch, &
...
!
   end subroutine L_pointer_setup_mo_mo_eri_tool
!
!
   subroutine cleanup_mo_eri_tool(eri)
!!
!!    Cleanup
...
!
      ...
      eri%mo_eri_mem = .false.
!
   end subroutine cleanup_mo_eri_tool
!
!
end module mo_eri_tool_class
