...
!
!           L_J_ij_t1 = L_J_ij_mo
            call eri%get_cholesky_mo(L_J_oo, batch_o%first, batch_o%get_last(), 1, eri%n_o)
!
!           L_J_ij_t1 += sum_b L_J_ib_mo t_bj
...
!
            call eri%set_cholesky_t1(L_J_oo, batch_o%first, batch_o%get_last(), 1, eri%n_o)
!
         enddo
!
...
!              L'_J_ji += L_J_ji_mo
               ...
               call eri%get_cholesky_mo(L_J_ij_p, 1, eri%n_o, &
                                        batch_o%first, batch_o%get_last())
!
...
!
            call eri%set_cholesky_t1(L_J_ai_p, &
                                     eri%n_o+batch_v%first, eri%n_o+batch_v%get_last(), 1, eri%n_o)
!
         enddo
!
...
!           L_J_ab_t1 -= sum_j t_aj L_J_jb_mo
            call eri%get_cholesky_mo(L_J_xv, &
                                     ...
                                     eri%n_o + batch_v%get_last(), 1, eri%n_o)
!
            call zgemm('N', 'T',                                       &
                       eri%n_J*batch_v%length, eri%n_v, eri%n_o, &
                       -one_complex,                                   &
